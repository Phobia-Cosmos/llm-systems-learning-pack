...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        # register_buffer 注册的是“跟随模型移动设备但不是可训练参数”的张量。手写 attention 保留 [1,1,T,T]
        # bool mask；SDPA 训练直接使用 is_causal=True，只保留空 buffer，KV-cache 路径按当前 query/key 长度生成 mask。
        # 这样 8K 上下文不会在每层预分配一份 float32 方阵。persistent=False 表示 mask 不进入 checkpoint。
        mask = (
            torch.empty((1, 1, 0, 0), dtype=torch.bool)
            if config.use_sdpa
            else torch.tril(
    ...
    def causal_mask_slice(
        ...
        if query_start < 0 or query_len <= 0 or key_len <= 0:
            raise ValueError("invalid causal mask range")
        if query_start + query_len > self.max_seq_len or key_len > self.max_seq_len:
            raise ValueError("causal mask range exceeds block_size")
        if self.causal_mask.numel() > 0:
            return self.causal_mask[
                :, :, query_start : query_start + query_len, :key_len
            ]
        ...
        return (key_positions.unsqueeze(0) <= query_positions.unsqueeze(1)).view(
    ...
    def forward(self, x: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_sdpa and position_bias is None:
        ...
        position_bias = self.position_encoding.attention_bias(
            positions, positions, dtype=scores.dtype, device=scores.device
        )
        ...
        causal_mask = self.causal_mask_slice(0, seq_len, seq_len, scores.device)
    ...
    def forward_with_cache(
        ...
        if position_bias is not None:
            scores = scores + position_bias
        mask = self.causal_mask_slice(past_len, seq_len, total_len, scores.device)
    ...
    def forward_with_static_cache(
        ...
        if total_len > key_cache.size(2) or total_len > self.max_seq_len:
    ...
    def __init__(self, config: GPTConfig):
        ...
        if config.scale_residual_init:
            ...
            for name, parameter in self.named_parameters():
                if name.endswith(("attn.c_proj.weight", "mlp.down_proj.weight")):
    ...
    def forward(
        ...
        for block in self.blocks:
            if self.gradient_checkpointing and self.training:
                x = checkpoint(block, x, positions, use_reentrant=False)
            else:
                x = block(x, positions)
        x = self.ln_f(x)
